...
function StatCard({ label, value, icon: Icon, color, suffix = "" }: {
  ...
  return (
    <motion.div variants={item} className="relative overflow-hidden rounded-2xl border border-white/5 bg-card p-6 backdrop-blur-sm">
      ...
      <div className="relative">
        ...
        <div className="text-3xl font-bold text-white mb-1">
          {value == null ? <Skeleton className="h-9 w-16" /> : `${value}${suffix}`}
        </div>
    ...
    <div className="flex-1 overflow-auto">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Here's your prompt engineering overview</p>
        </motion.div>
        ...
        >
          <StatCard label="Total Prompts" value={stats?.totalPrompts} icon={Sparkles} color="from-purple-500 to-violet-600" />
          <StatCard label="Favorites" value={stats?.totalFavorites} icon={Star} color="from-amber-500 to-orange-500" />
          <StatCard label="Total Copies" value={stats?.totalCopies} icon={Copy} color="from-blue-500 to-cyan-500" />
          <StatCard label="This Week" value={stats?.promptsThisWeek} icon={Calendar} color="from-emerald-500 to-teal-500" />
        </motion.div>
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="rounded-xl border border-white/5 bg-card p-8 text-center">
                  ...
                  <Link href="/generate">
                    <button className="text-sm font-medium bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 px-4 py-2 rounded-lg transition-colors">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 flex items-center gap-1 text-xs text-primary hover:text-primary/80 transition-all"
                      >
          ...
          >
            ...
            <Link href="/generate">
              <motion.div
                ...
                className="rounded-xl bg-gradient-to-br from-primary/20 to-purple-900/30 border border-primary/20 p-5 cursor-pointer"
              >
